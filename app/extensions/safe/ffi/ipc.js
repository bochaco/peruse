...
const openExternal = ( uri ) =>
{
    ...
    try
    {
        logger.info('in open external eh')
...
class ReqQueue
{
    ...
    process()
    {
        ...
        } ).catch( ( err ) =>
        {
            ...
            logger.error( 'uriii',this.req.uri  );
            logger.error( err );
            if ( ipcEvent )
            {
...
const decodeRequest = ( e, req, type ) =>
{
    ...
    const request = new Request( {
        id  : req.id,
        uri : req.uri,
        // uri : isWebReq ? req.uri : req, //why this check?
        type,
        isUnRegistered
    } );
...
const onAuthDecision = ( e, authData, isAllowed ) =>
{
    ...
    authenticator.encodeAuthResp( authData, isAllowed )
        .then( ( res ) =>
        {
            logger.info('there is a response!!!!', res)
            reqQ.req.res = res;
